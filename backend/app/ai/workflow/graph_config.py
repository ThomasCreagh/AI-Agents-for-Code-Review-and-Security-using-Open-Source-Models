...
def rag_agent(state: AgentState, vector_store, llm):
    ...
    doc_sources = [getattr(doc, 'metadata', {}).get(
...
def security_analysis_agent(state: AgentState, llm):
    ...
    if doc_sources:
        log_debug(
            f"Security analysis using documents from: {', '.join(doc_sources)}")
...
def create_security_rag_graph(llm, vector_store):
    ...
    workflow.add_node(
        "security", lambda state: security_analysis_agent(state, llm))
    workflow.add_node("generate_response",
                      lambda state: generate_response(state, llm))
