...
const apiRequest = async (endpoint, method = 'GET', body = null, files = null) => {
  const headers = {
...
export const submitCodeForReview = async (codeFile, securityContext = null, language = 'python', referenceDocuments = "false") => {
  ...
  return apiRequest('/ast-analysis/submit-code-for-review', 'POST', formData, { code_file: codeFile });
};

export const reviewCodeFile = async (codeFiles, errorDescription = '', language = 'python', model = null) => {
  const formData = {
  ...
  return apiRequest('/code-review/file', 'POST', formData, { code_files: codeFiles });
};
...
export const uploadDocument = async (documentFile) => {
  return apiRequest('/documents/upload', 'POST', {}, { document_file: documentFile });
};